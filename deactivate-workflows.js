/**
 * deactivate-workflows.js
 *
 * Deactivates workflows on the n8n server.
 *
 * Usage:
 *   node deactivate-workflows.js                         # deactivate all adapter workflows
 *   node deactivate-workflows.js --workflow my-flow       # deactivate one by name (omit .json)
 *   node deactivate-workflows.js --all                    # deactivate ALL built workflows
 *   node deactivate-workflows.js --dry-run                # show what would happen
 *
...
// Determine which workflow names to deactivate
let targetNames;
if (targetName) {
...
for (const name of targetNames) {
  ...
  if (dryRun) {
    console.log(`  [dry-run] "${name}" (${sw.id}) — would deactivate`);
    deactivated++;
    continue;
  }
  try {
    const res = await fetch(new URL(`/api/v1/workflows/${sw.id}/deactivate`, BASE_URL), {
      method: 'POST',
    ...
    console.log(`  "${name}" (${sw.id}) — deactivated`);
    deactivated++;
  } catch (err) {
...
console.log(`\nDone. ${deactivated} deactivated, ${skipped} already inactive, ${errors} error(s).`);
